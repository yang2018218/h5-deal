		...
		<view class="content" @touchstart="hideDrawer">
			<scroll-view class="msg-list" scroll-y="true" :scroll-with-animation="scrollAnimation" :scroll-top="scrollTop"
			 :scroll-into-view="scrollToView" @scrolltoupper="loadHistory" upper-threshold="50">
				...
				<view class="row" v-for="(item,index) in this.msgList" :key="index" :id="item.ID || 0">
					<!-- 用户消息 -->
					<block>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			//防止初次聊天的时候 没有对方的基础信息
			console.log(userList,"6666")
			userList.forEach((item)=>{
					console.log(item,this.toUserId,"55555")
